<h2 class="text-2xl md:text-3xl font-extrabold text-white mb-8 text-center animate-fade-in-up">Cumulative Flight Hours and Take Off</h2>

<form action="{{ url('/report/cumulative') }}" method="POST" class="bg-white p-8 rounded-3xl shadow-2xl border backdrop-blur-md animate-fade-in-up delay-100">
    @csrf
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-10 animate-fade-in-up delay-200">
        ...
        <div class="w-full md:w-1/3 animate-fade-in-up delay-500">
            <label for="aircraft_type" class="block text-base font-bold mb-2 tracking-wide text-[#0572a6]">AC Type</label>
...
document.getElementById('operator').addEventListener('change', function () {
    ...
    const aircraftTypeDropdown = document.getElementById('aircraft_type');
